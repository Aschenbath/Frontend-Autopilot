...
## 修复前后对比：同一页面，第 0 轮与第 1 轮

第一版 SignalDesk 看起来已经完整，但本地 UI 检查在桌面和手机两个视口下抓出 49 项失败：会话编号链接点击区域只有 49×15 px，风险标签和状态文字的对比度在 3.3 到 3.7 之间，导航标签和搜索框也低于 44 px。Fixer 只拿到这些失败证据，只改了一份 CSS，然后整个应用重新走一遍构建、场景和 UI 检查。

![SignalDesk 会话队列桌面视口修复前后对比](docs/screenshots/signaldesk-repair-desktop.png)

![SignalDesk 会话队列手机视口修复前后对比](docs/screenshots/signaldesk-repair-mobile.png)

| | 第 0 轮 | 第 1 轮 |
| --- | --- | --- |
| UI 检查失败项（全部页面与视口） | 49 | 0 |
| 最小点击尺寸 44 px | 链接 49×15、导航 76×38、搜索框高 42 | 全部通过 |
| 文字对比度 4.5 | 高风险 3.72、中风险 3.33、已复核 3.49 | 全部通过 |
| 6 个交互场景 | 通过 | 仍然通过 |
| 改动范围 | | 仅 `src/styles.css` |

两张对比图里的截图都由 Playwright 在验收时自动采集，数字来自 `quality/round-0.json` 与 `round-1.json`，不是事后手工整理。
...
另一个带参考图的 Atlas Research 挑战没有通过最终视觉与 UI 检查，所以仓库没有把它包装成成功案例。它暴露的问题比成功案例更有意思。

![Atlas Research 被拒绝交付的三轮对比](docs/screenshots/atlas-rejected.png)

第 0 轮模型做出了一个正确的研究资料库，但和用户提供的参考图结构相似度太低。到第 2 轮，Fixer 为了拉高相似度，直接把产品改成了参考图里的便签应用，它自己的诊断写的是“我将资料库和阅读页改为目标便签式布局”。这是模型在追指标而不是做产品。分数只从 0.33 涨到 0.42，仍未过线，同时 57 项 UI 检查失败，运行以 failed 结束。

这件事说明两点。验收器没有被“看起来更像参考图”骗过去，这是它该做的。但把参考图直接交给 Fixer 会诱导它复制而不是修复，这是当前设计的一个真实缺陷，已经记录为后续要改的地方。Frontend Autopilot 的价值不是保证模型每次都能做出好产品，而是让“不够好”变成可发现、可解释、会被拒绝的结果。

目前项目只生成使用模拟数据的 React + Vite 前端，不包含真实后端、登录系统、多人协作或云端部署，也不宣称能代替人工审美判断。

## 对照：同一套“看截图写前端”的思路，手工 vibe coding 的浏览器插件

下面几张不是 Frontend Autopilot 的产出，而是我另外用同样的截图驱动方式手工 vibe coding 出来的两个浏览器插件（课程表、稍后阅读）：把目标界面截图交给模型，逐轮对照截图修正。放在这里是为了对照两条路线的差距：手工路线能拿到更精细的视觉结果，但每一轮的验收都靠人眼；Frontend Autopilot 追求的是把“跑起来、点一遍、量一遍”这一段交给本地程序，代价是目前的生成质量还明显不如人工盯着改。
...
| Read It Later 稍后阅读列表 |
| --- |
| ![Read It Later 插件列表页](docs/screenshots/read-it-later-list.png) |
